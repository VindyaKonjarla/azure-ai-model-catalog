...
if __name__ == "__main__":
    ...
    #or test_sku_type is None or test_queue is None or test_set is None or test_trigger_next_model is None or test_keep_looping is None:
    ...
    try:
        ...
        # Fall back to InteractiveBrowserCredential in case DefaultAzureCredential not work
        credential = InteractiveBrowserCredential()
    #logger.info(f"workspace_name : {workspace}")
    try:
        workspace_ml_client = MLClient.from_config(credential=credential)
    except:
        workspace_ml_client = MLClient(
    ...
    environment_variables = {"AZUREML_ARTIFACTS_DEFAULT_TIMEOUT":600.0,"test_model_name": test_model_name}
    env_list = workspace_ml_client.environments.list(name="environment")
    latest_version = 0
    for env in env_list:
        if latest_version <= int(env.version):
            latest_version = int(env.version)
    logger.info(f"Latest Environment Version: {latest_version}")
    latest_env = workspace_ml_client.environments.get(
        name="environment", version=str(latest_version))
    logger.info(f"Latest Environment : {latest_env}")
    command_job = run_azure_ml_job(code="./", command_to_run="python generic_model_download_and_register.py",
                                   environment=latest_env, compute="compute", environment_variables=environment_variables)
    create_and_get_job_studio_url(command_job, workspace_ml_client)
