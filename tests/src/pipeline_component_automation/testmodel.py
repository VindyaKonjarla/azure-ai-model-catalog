...
# file the index of model_name in models list queue dictionary
    model_list = list(queue.models)
    #model_name_without_slash = model_name.replace('/', '-')
    check_mlflow_model = "MLFlow-"+model_name
    import_alias_model_name = f"MLFlow-Import-{model_name}"
    ...
    if check_mlflow_model in model_list:
        ...
        index = model_list.index(model_name.replace("/", "-").lower()+"-oss")

    logger.info(f"index of {model_name} in queue: {index}")
...
def model_import_pipeline(compute_name, update_existing_model, task_name):
    ...
    import_model_job = import_model(model_id=model_name, compute=compute_name,
...
if __name__ == "__main__":
    ...
    if model_name is None or test_sku_type is None or test_queue is None or test_set is None or test_trigger_next_model is None or test_keep_looping is None:
        logger.error("::error:: One or more of the environment variables model_name, test_sku_type, test_queue, test_set, test_trigger_next_model, test_keep_looping are not set")
        exit(1)
    ...
    for model_name in model_list:
        ...
        try:
            ...
            schedule_huggingface_model_import = (
                ...
                and model_name not in [None, "None"]
                and len(model_name) > 1
            )
            logger.info(
                f"Need to schedule run for importing {model_name}: {schedule_huggingface_model_import}")
